...
def get_castling_king_target_square(move: CastlingMove) -> Coordinate:
    if move.side is CastlingSide.KINGSIDE:
...
def get_castling_rook_target_square(move: CastlingMove) -> Coordinate:
    if move.side is CastlingSide.KINGSIDE:
        rook_file = notation.get_file_by_char("f")
    else:
        rook_file = notation.get_file_by_char("d")
...
def get_intermediary_squares(square_a: Coordinate, square_b: Coordinate) -> Set[Coordinate]:
    if square_a.rank == square_b.rank:
        intermediary_files = _get_numbers_between(square_a.file, square_b.file)

        return set(Coordinate(file, square_a.rank) for file in intermediary_files)
    elif square_a.file == square_b.file:
        intermediary_ranks = _get_numbers_between(square_a.rank, square_b.rank)

        return set(Coordinate(square_a.file, rank) for rank in intermediary_ranks)
    else:
        raise ValueError("cannot get intermediary squares: square_a and square_b don't share file nor rank")


def get_castling_intermediary_squares(move: CastlingMove) -> Set[Coordinate]:
...
def get_castling_passthrough_squares(move: CastlingMove) -> Set[Coordinate]:
    king_square = get_castling_king_square(move)
    king_target_square = get_castling_king_target_square(move)

    return get_intermediary_squares(king_square, king_target_square)
